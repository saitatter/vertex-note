...
class NotePage: public PageHandler {
public:
    NotePage(double width, double height, bool suppressLayerCreation = false);
    ~NotePage() override;
    NotePage(const NotePage& page);
    void operator=(const NotePage& p) = delete;

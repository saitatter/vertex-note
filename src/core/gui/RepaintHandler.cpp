...
RepaintHandler::RepaintHandler(VertexNoteView* noteView): noteView(noteView) {}

RepaintHandler::~RepaintHandler() { this->noteView = nullptr; }
...
void RepaintHandler::repaintPage(const PageView* view) {
    ...
    gtk_vertex_note_repaint_area(this->noteView->getWidget(), p.x, p.y, x2, y2);
}

void RepaintHandler::repaintPageArea(const PageView* view, int x1, int y1, int x2, int y2) {
    auto p = view->getPixelPosition();
    gtk_vertex_note_repaint_area(this->noteView->getWidget(), p.x + x1, p.y + y1, p.x + x2, p.y + y2);
}

void RepaintHandler::repaintPageBorder(const PageView* view) { gtk_widget_queue_draw(this->noteView->getWidget()); }

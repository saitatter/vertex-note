...
#include "gui/widgets/VertexNoteWidget.h"           // for gtk_vertex_note_get_layout
...
std::pair<size_t, size_t> VertexNoteView::preloadPageBounds(size_t page, size_t maxPage) {
...
VertexNoteView::VertexNoteView(GtkWidget* parent, Control* control, ScrollHandling* scrollHandling):
    ...
    this->widget = gtk_vertex_note_new(this, inputContext, scrollHandling->getVertical(), scrollHandling->getHorizontal());
...
auto VertexNoteView::clearMemoryTimer(VertexNoteView* widget) -> gboolean {
    widget->cleanupBufferCache();
    return G_SOURCE_CONTINUE;
}

auto VertexNoteView::cleanupBufferCache() -> void {
...
auto VertexNoteView::getCurrentPage() const -> size_t { return currentPage; }

const int scrollKeySize = 30;

auto VertexNoteView::onKeyPressEvent(const KeyEvent& event) -> bool {
    ...
    if (auto* geom = GTK_VERTEX_NOTE(widget)->input->getGeometryToolInputHandler(); geom && geom->keyPressed(event)) {
        return true;
    }
...
auto VertexNoteView::getRepaintHandler() const -> RepaintHandler* { return this->repaintHandler.get(); }

auto VertexNoteView::onKeyReleaseEvent(const KeyEvent& event) -> bool {
    size_t p = getCurrentPage();
    if (p != npos && p < this->viewPages.size()) {
        auto& v = this->viewPages[p];
...
void VertexNoteView::onSettingsChanged() {
    if (this->cache) {
...
void VertexNoteView::requestFocus() { gtk_widget_grab_focus(this->widget); }

auto VertexNoteView::searchTextOnPage(const std::string& text, size_t pageNumber, size_t index, size_t* occurrences,
...
void VertexNoteView::forceUpdatePagenumbers() {
...
auto VertexNoteView::getViewFor(size_t pageNr) const -> PageView* {
...
void VertexNoteView::pageSelected(size_t page) {
...
auto VertexNoteView::getControl() const -> Control* { return control; }

void VertexNoteView::scrollTo(size_t pageNo, PdfRectangle rect) {
    double zoom = getZoom();
...
void VertexNoteView::pageRelativeXY(int offCol, int offRow) {
    Layout* layout = this->getLayout();
...
void VertexNoteView::endTextAllPages(PageView* except) const {
...
void VertexNoteView::endPendingInputAllPages() const {
    for (auto& v: this->viewPages) {
        v->endPendingInput();
    }
}

void VertexNoteView::layerChanged(size_t page) {
    if (page != npos && page < this->viewPages.size()) {
        this->viewPages[page]->rerenderPage();
    }
}

void VertexNoteView::getPasteTarget(double& x, double& y) const {
...
auto VertexNoteView::getVisibleRect(size_t page) const -> Rectangle<double>* {
    if (page == npos || page >= this->viewPages.size()) {
        return nullptr;
    }
...
auto VertexNoteView::getVisibleRect(const PageView* redrawable) const -> Rectangle<double>* {
    return gtk_vertex_note_get_visible_area(this->widget, redrawable);
}

void VertexNoteView::recreatePdfCache() {
    this->cache.reset();
 ...
 */
auto VertexNoteView::getHandRecognition() const -> HandRecognition* {
    return GTK_VERTEX_NOTE(widget)->input->getHandRecognition();
}

/**
 * @return Scrollbars
 */
auto VertexNoteView::getScrollHandling() const -> ScrollHandling* { return scrollHandling; }

auto VertexNoteView::getWidget() const -> GtkWidget* { return widget; }

void VertexNoteView::ensureRectIsVisible(int x, int y, int width, int height) {
    Layout* layout = this->getLayout();
    layout->ensureRectIsVisible(x, y, width, height);
}

void VertexNoteView::zoomChanged() {
...
void VertexNoteView::pageSizeChanged(size_t page) {
    layoutPages();
    if (page != npos && page < this->viewPages.size()) {
...
void VertexNoteView::pageChanged(size_t page) {
    if (page != npos && page < this->viewPages.size()) {
        this->viewPages[page]->rerenderPage();
    }
}

void VertexNoteView::pageDeleted(size_t page) {
...
auto VertexNoteView::getTextEditor() const -> TextEditor* {
...
auto VertexNoteView::getCache() const -> PdfCache* { return this->cache.get(); }

void VertexNoteView::pageInserted(size_t page) {
    Document* doc = control->getDocument();
    doc->lock_shared();
    auto pageView = std::make_unique<PageView>(this, doc->getPage(page));
    doc->unlock_shared();
...
auto VertexNoteView::getZoom() const -> double { return control->getZoomControl()->getZoom(); }

auto VertexNoteView::getDpiScaleFactor() const -> int { return gtk_widget_get_scale_factor(widget); }

void VertexNoteView::clearSelection() {
    EditSelection* sel = GTK_VERTEX_NOTE(widget)->selection;
    GTK_VERTEX_NOTE(widget)->selection = nullptr;
    delete sel;
...
void VertexNoteView::deleteSelection(EditSelection* sel) {
    if (sel == nullptr) {
        sel = getSelection();
    }
...
void VertexNoteView::setSelection(EditSelection* selection) {
    clearSelection();
    GTK_VERTEX_NOTE(this->widget)->selection = selection;
...
void VertexNoteView::repaintSelection(bool evenWithoutSelection) {
...
void VertexNoteView::layoutPages() { this->getLayout()->recalculate(); }

auto VertexNoteView::getDisplayHeight() const -> int {
    GtkAllocation allocation = {0};
    gtk_widget_get_allocation(this->widget, &allocation);
    return allocation.height;
}

auto VertexNoteView::getDisplayWidth() const -> int {
    GtkAllocation allocation = {0};
    gtk_widget_get_allocation(this->widget, &allocation);
    return allocation.width;
}

auto VertexNoteView::isPageVisible(size_t page, int* visibleHeight) const -> bool {
...
void VertexNoteView::documentChanged(DocumentChangeType type) {
    if (type != DOCUMENT_CHANGE_CLEARED && type != DOCUMENT_CHANGE_COMPLETE) {
        return;
    }

    VertexNoteScheduler* scheduler = this->control->getScheduler();
    scheduler->lock();
    ...
    for (size_t i = 0; i < pagecount; i++) {
        viewPages.emplace_back(std::make_unique<PageView>(this, doc->getPage(i)));
    }
...
auto VertexNoteView::paste() -> bool {
    size_t p = getCurrentPage();
    if (p == npos || p >= viewPages.size()) {
        return false;
    }
...
auto VertexNoteView::actionDelete() -> bool {
    size_t p = getCurrentPage();
    if (p == npos || p >= viewPages.size()) {
        return false;
    }
...
auto VertexNoteView::getDocument() const -> Document* { return control->getDocument(); }

auto VertexNoteView::getViewPages() const -> std::vector<std::unique_ptr<PageView>> const& { return viewPages; }

auto VertexNoteView::getCursor() const -> VertexNoteCursor* { return control->getCursor(); }

auto VertexNoteView::getSelection() const -> EditSelection* {
    g_return_val_if_fail(this->widget != nullptr, nullptr);
    g_return_val_if_fail(GTK_IS_VERTEX_NOTE(this->widget), nullptr);

    return GTK_VERTEX_NOTE(this->widget)->selection;
}

auto VertexNoteView::getLayout() const -> Layout* { return gtk_vertex_note_get_layout(getWidget()); }

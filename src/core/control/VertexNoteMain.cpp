...
auto migrateSettings() -> MigrateResult {
    ...
    if (!fs::exists(newConfigPath)) {
        const std::array oldPaths = {
                Util::getConfigFolder().parent_path() / "com.github.vertex-note.vertex-note",
                Util::getConfigFolder().parent_path() / "com.github.vertex-note.vertex-note.exe",
                Util::GFilename(g_get_home_dir()).toPath().value_or(fs::path()) / ".vertex-note",
        };
...
static void deleteFile(const fs::path& file, GtkWindow* win) {
    ...
    if (!fs::remove(file, error)) {
        ...
        AppMessageBox::showErrorToUser(win, msg.str());
    }
}
...
struct VertexNoteMainPrivate {
    VertexNoteMainPrivate() = default;
    VertexNoteMainPrivate(VertexNoteMainPrivate&&) = delete;
    VertexNoteMainPrivate(VertexNoteMainPrivate const&) = delete;
    auto operator=(VertexNoteMainPrivate&&) -> VertexNoteMainPrivate = delete;
    auto operator=(VertexNoteMainPrivate const&) -> VertexNoteMainPrivate = delete;

    ~VertexNoteMainPrivate() {
...
using XMPtr = VertexNoteMainPrivate*;
...
void initResourcePath(GladeSearchpath* gladePath, const gchar* relativePathAndFile, bool failIfNotFound) {
    ...
    if (!failIfNotFound) {
        ...
        AppMessageBox::showErrorAndQuit(msg, 12);
    }
}
...
gint on_command_line(GApplication*, GApplicationCommandLine*, XMPtr) {
    g_message("VertexNoteMain::on_command_line: This should never happen, please file a bugreport with a detailed "
...
void on_open_files(GApplication* application, gpointer f, gint numFiles, gchar* hint, XMPtr app_data) {
    ...
    if (numFiles != 1) {
        const std::string msg = _("Sorry, VertexNote can only open one file at once.\n"
                                  "Others are ignored.");
        AppMessageBox::showErrorToUser(GTK_WINDOW(app_data->win->getWindow()), msg);
    }
...
void on_startup(GApplication* application, XMPtr app_data) {
    ...
    if (migrateResult.status != MigrateStatus::NotNeeded) {
        Util::execInUiThread(
                [=]() { AppMessageBox::showErrorToUser(app_data->control->getGtkWindow(), migrateResult.message); });
    }
...
void VertexNoteMain::initLocalisation() {
#ifdef ENABLE_NLS
    ...
    try {
        ...
        g_warning("VertexNoteMain: System default locale could not be set.\n - Caused by: %s\n - Note that it is not "
...
auto VertexNoteMain::run(int argc, char** argv) -> int {

    VertexNoteMainPrivate app_data;

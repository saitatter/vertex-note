...
VertexNoteScheduler::VertexNoteScheduler() { this->name = "VertexNoteScheduler"; }

VertexNoteScheduler::~VertexNoteScheduler() = default;

void VertexNoteScheduler::removeSidebar(SidebarPreviewBaseEntry* preview) {
...
void VertexNoteScheduler::removePage(PageView* view) { removeSource(view, JOB_TYPE_RENDER, JOB_PRIORITY_URGENT); }

void VertexNoteScheduler::removeAllJobs() {
...
void VertexNoteScheduler::finishTask() { std::lock_guard lock{this->jobRunningMutex}; }

void VertexNoteScheduler::removeSource(void* source, JobType type, JobPriority priority, bool awaitFinishTask) {
    {
        std::lock_guard lock{this->jobQueueMutex};
        std::deque<Job*>& queue = *this->jobQueue[priority];
...
auto VertexNoteScheduler::existsSource(void* source, JobType type, JobPriority priority) -> bool {
    bool exists = false;
    std::lock_guard lock{this->jobQueueMutex};
...
void VertexNoteScheduler::addRepaintSidebar(SidebarPreviewBaseEntry* preview) {
...
void VertexNoteScheduler::addRerenderPage(PageView* view) {

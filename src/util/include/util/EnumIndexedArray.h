...
template <class T, typename enum_class,
          std::enable_if_t<std::is_enum_v<enum_class> && std::is_unsigned_v<std::underlying_type_t<enum_class>> &&
                           ...
                           bool> = true>
class EnumIndexedArray: private std::array<T, vn::to_underlying(enum_class::ENUMERATOR_COUNT)> {
public:
    using underlying_array_type = std::array<T, vn::to_underlying(enum_class::ENUMERATOR_COUNT)>;
    ...
    constexpr T& operator[](enum_class value) {
        ...
        return underlying_array_type::operator[](vn::to_underlying(value));
    }

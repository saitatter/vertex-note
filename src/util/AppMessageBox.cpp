...
#ifdef _WIN32
// Needed for help dialog workaround on Windows; see AppMessageBox::showHelp
#include <windows.h>
...
AppMessageBox::AppMessageBox(GtkDialog* dialog, vn::util::move_only_function<void(int)> callback, CallbackPolicy pol):
    ...
    this->signalId =
            g_signal_connect(dialog, "response", G_CALLBACK(+[](GtkDialog* dialog, int response, gpointer data) {
                                 auto* self = static_cast<AppMessageBox*>(data);
 ...
 */
void AppMessageBox::setDefaultWindow(GtkWindow* win) { defaultWindow = win; }

void AppMessageBox::showMarkupMessageToUser(GtkWindow* win, const std::string_view& markupTitle, const std::string& msg,
                                        GtkMessageType type) {
    if (win == nullptr) {
        win = defaultWindow;
    }

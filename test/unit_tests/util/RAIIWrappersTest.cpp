...
TEST(UtilsRAII, testGObjectSPtrConstructors) {
    using TestPtr = vn::util::GObjectSPtr<TestObject>;
    EXPECT_EQ(TestObjectCount, 0);
    {
        TestPtr t(TEST_OBJECT(g_object_new(TEST_OBJECT_TYPE, nullptr)), vn::util::adopt);
    ...
    EXPECT_EQ(TestObjectCount, 0);
    {
        ...
        EXPECT_EQ(TestObjectCount, 1);
        TestPtr t1(t, vn::util::adopt);  // Adopt a floating ref
        EXPECT_EQ(t1.get(), t);
...
TEST(UtilsRAII, testGObjectSPtrReset) {
    ...
    {
        TestPtr t;
        t.reset(TEST_OBJECT(g_object_new(TEST_OBJECT_TYPE, nullptr)), vn::util::adopt);
        EXPECT_FALSE(g_object_is_floating(t.get()));
        EXPECT_EQ(TestObjectCount, 1);
    }
    EXPECT_EQ(TestObjectCount, 0);
    {
        ...
        EXPECT_EQ(TestObjectCount, 1);
        TestPtr t1;
        t1.reset(t, vn::util::adopt);  // Adopt a floating ref
        EXPECT_EQ(t1.get(), t);
        EXPECT_FALSE(g_object_is_floating(t1.get()));
    }
    EXPECT_EQ(TestObjectCount, 0);
